...
(i₁ : [m] ⟶ [n]) : i₁ ≫ s₁ = 𝟙 [m] → i₁ ≫ s₂ = 𝟙 [m] :=
begin
  intro hi₁,
  let u := i₁ ≫ s₂, change u = 𝟙 [m],
  have hu : (X.map u.op) y₂ = y₁,
  { rw [op_comp, X.map_comp, types_comp_apply, ←hy₁y₂],
    rw [←types_comp_apply (X.map s₁.op) (X.map i₁.op)],
    rw [←X.map_comp, ←op_comp,hi₁, op_id, X.map_id, types_id_apply], },
  ...
  rw hs₃i₃,
  exact @id_of_iso _ _ (@is_iso.comp_is_iso _ _ _ _ _ _ _
...
begin
  ...
  { cases split_epi_of_degeneracy hs₁ with i₁ hi₁,
    simp only [auto_param_eq] at *,
    have hu : i₁ ≫ s₂ = 𝟙 [m], from same_sections X x y₁ y₂ hy₁ hy₂ hy₁y₂ i₁ hi₁,
    let u := i₁ ≫ s₂, change u = 𝟙 [m] at hu,
    have hu' : (X.map u.op) y₂ = y₁,
    { rw [op_comp, X.map_comp, types_comp_apply, ←hy₁y₂],
      rw [←types_comp_apply (X.map s₁.op) (X.map i₁.op)],
      rw [←X.map_comp, ←op_comp,hi₁, op_id, X.map_id, types_id_apply], },
